...
def get_plant_data(url, plant_id: int) -> dict:
    ...
    response = req.get(f'{url}{plant_id}', timeout=20)
...
def extract_country_data(api_data_dict: dict) -> list:
    """Extracts Country Data from the api_data dictionary."""
    country_list = []
    seen_countries = set()

    for plant in api_data_dict.values():
...
def extract_city_data(api_data_dict: dict, country_map_dict: dict) -> list:
    ...
    for plant in api_data_dict.values():
        ...
        country_code = plant.get(
            'origin_location', [None, None, None, None, None])[3]
        country_id = country_map_dict.get(country_code)
...
def extract_origin_location_data(api_data_dict: dict, city_map_dict: dict) -> list:
    ...
    for plant in api_data_dict.values():
        ...
        city_id = city_map_dict.get(city_name)

        if (latitude, longitude) not in seen_origin_location:
            origin_location_list.append((latitude, longitude, city_id))
            seen_origin_location.add((latitude, longitude))

    return origin_location_list


def extract_plant_data(api_data_dict: dict, origin_location_map_dict: dict) -> list:
    ...
    for plant in api_data_dict.values():
        ...
        lat_lng_str = f'{('%.3f' % latitude).rstrip('0').rstrip('.')},{
            ('%.3f' % longitude).rstrip('0').rstrip('.')}'

        origin_location_id = origin_location_map_dict.get(lat_lng_str)
...
def extract_botany_data(api_data_dict: dict) -> list:
...
if __name__ == "__main__":
    ...
    country_map = get_id_mapping(
        conn,
        "SELECT country_code, country_id FROM country")
    ...
    load_into_db(
        conn,
        cities,
        "INSERT INTO city (city_name, country_id, time_zone) VALUES (%s, %s, %s)")
    ...
    load_into_db(
        conn,
        origin_location,
        "INSERT INTO origin_location (latitude, longitude, city_id) VALUES (%s, %s, %s)")

    origin_location_map = get_id_mapping(
        conn,
        """
        SELECT 
            CONCAT(ROUND(latitude, 3), ',', ROUND(longitude, 3)) AS lat_long, 
            origin_location_id 
        FROM 
            origin_location""")
